...
module.exports = {
	...
	async execute(interaction) {
        ...
        if(!isConnected) return await interaction.reply({ content: 'There are no songs playing', ephemeral: true });

        /* Checking if the music is already paused. If it is, return. */
        const isPaused = await music.isPaused({
            interaction: interaction
        });
        if(isPaused) return await interaction.reply({ content: 'The song is already paused', ephemeral: true });
